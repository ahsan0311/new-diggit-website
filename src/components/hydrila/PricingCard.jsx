...
const PricingCard = ({
  ...
  return (
    <div className="w-full h-[850px] rounded-2xl bg-gradient-to-r from-cyan-300 to-blue-300 p-6 flex flex-col">
      {/* Header */}
      ...
      {/* Features (no internal scrolling) */}
...
const PricingPlans = ({
  ...
  const currentPlans = activePlan === "standard" ? plansStandard : plansPerformance;
...
>
          ...
          {currentPlans.map((plan, idx) => (
            <SwiperSlide key={idx}>
              <PricingCard {...plan} showButton={showButtonInCard} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
